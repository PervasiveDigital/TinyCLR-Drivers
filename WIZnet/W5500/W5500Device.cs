...
namespace GHIElectronics.TinyCLR.Drivers.WIZnet.W5500 {
    ...
    internal class W5500Device {
        ...
        public W5500Device(int sockets, SpiController controller, int csPinId, int resetPinId = -1, int intPinId = -1) {
            ...
            if (resetPinId != -1) {
                this.resetPin = GpioController.GetDefault().OpenPin(resetPinId);
                this.resetPin.SetDriveMode(GpioPinDriveMode.Output);
            }

            if (intPinId != -1) {
                this.interruptPin = GpioController.GetDefault().OpenPin(resetPinId);
            ...
            var cs = GpioController.GetDefault().OpenPin(csPinId);
        ...
        internal byte RegisterRead(Registers reg) {
            ...
            this.spi.TransferSequential(this.buffer, 0, 3, this.buffer, 3, 1);
        ...
        internal void RegisterRead(Registers reg, byte[] data, int offset, int len) {
            ...
            this.buffer[1] = (byte)(((uint)reg >> 8) & 0xff);
            this.buffer[2] = (byte)(((uint)reg & 0xf8) | SPI_READ);
            this.spi.TransferSequential(this.buffer, 0, 3, data, offset, len);
        }
        ...
        internal ushort SocketRegisterReadUshort(SocketRegisters reg, int socket) {
            ...
            this.buffer[1] = (byte)(((uint)addr >> 8) & 0xff);
            this.buffer[2] = (byte)(((uint)addr & 0xf8) | SPI_READ);
            this.spi.TransferSequential(this.buffer, 0, 3, this.buffer, 3, 2);
        ...
        internal void RegisterWrite(Registers reg, byte data) {
            ...
            this.buffer[3] = data;
            this.spi.TransferSequential(this.buffer, 0, 4, this.buffer, 4, 0);
        }
        ...
        internal void RegisterWrite(Registers reg, params byte[] data) {
            ...
            this.spi.TransferSequential(this.buffer, 0, data.Length + 3, this.buffer, data.Length + 3, 0);
        }
        ...
        internal int WriteData(int slot, ref ushort ptr, byte[] src, int offset, int len) {
            ...
            this.spi.TransferSequential(this.buffer, 0, len + 3, this.buffer, 0, 0);
            ptr += (ushort)len;
        ...
        internal void ReadData(int slot, ref ushort ptr, byte[] buffer, int offset, int len) {
            ...
            this.spi.TransferSequential(this.buffer, 0, 3, buffer, 0, len);
            ptr += (ushort)len;
        }
        ...
        public void Reset() {
            ...
            if (this.interruptPin != null) {
                ...
                this.RegisterWrite(W5500Device.Registers.SIMR, (byte)0x0ff);
        ...
        private void InterruptPin_ValueChanged(GpioPin sender, GpioPinValueChangedEventArgs e) {
            var ir = this.RegisterRead(W5500Device.Registers.IR);

            if ((ir & (byte)IR.CONFLICT) != 0) {
                this.OnAddressConflictDetected?.Invoke(this);
                this.RegisterWrite(W5500Device.Registers.IR, (byte)IR.CONFLICT);
            }
            if ((ir & (byte)IR.MP) != 0) {
                this.OnWakeOnLanReceived?.Invoke(this);
                this.RegisterWrite(W5500Device.Registers.IR, (byte)IR.MP);
            }
            if ((ir & (byte)IR.PPPoE) != 0) {
                this.OnPPPoEClosed?.Invoke(this);
                this.RegisterWrite(W5500Device.Registers.IR, (byte)IR.PPPoE);
            }
            if ((ir & (byte)IR.UNREACHABLE) != 0) {
                this.OnAddressUnreachable?.Invoke(this);
                this.RegisterWrite(W5500Device.Registers.IR, (byte)IR.UNREACHABLE);
            }

            // Handle per-socket interrupts
            ir = this.RegisterRead(W5500Device.Registers.SIR);
            for (var i = 0; i < 8; ++i) {
                if ((ir & (1 << i)) != 0) {
        ...
        private void HandleSocketInterrupt(int socket) {
            var sir = this.SocketRegisterRead(W5500Device.SocketRegisters.IR, socket);
